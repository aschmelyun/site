<footer class="text-gray-700 body-font overflow-hidden mt-12">
        ...
        <div class="w-full lg:w-1/2 mb-10 px-4 text-center lg:text-right">
            ...
            <p>
                @foreach($footerLinks as $name => $link)
                    <a href="{{ $link }}" target="_blank" rel="noreferrer" class="text-gray-700 hover:text-gray-900 transition-colors duration-200">{{ $name }}</a>
                    @if($name !== 'Dev.to')
                        &nbsp;&mdash;&nbsp;
                    @endif
                @endforeach
